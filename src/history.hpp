    ...
    class History {
      public:
        ...
        void append_history_to_file(const std::string& history_file = "") {
            ...
            int itr_file =
                file_content.size() > 0 ? file_content.size() - 1 : 0; // we start reading from the end of file
        ...
        void print_last(unsigned number = 0) {
            ...
            for (auto it = cached_history.end() - number; it != cached_history.end() && number > 0;
                 ++it, ++original_index, --number) {
